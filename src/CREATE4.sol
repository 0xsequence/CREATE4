...
contract CREATE4 {
   ...
   */
  function packLeafPrefix(
    uint64 chainid,
    uint64 nextChainid,
    uint8 isFallback
  ) internal pure returns (bytes32) {
    return bytes32(
      uint256(isFallback) << 248 |
      uint256(nextChainid) << 64 |
      uint256(chainid)
    );
  }
   ...
   */
  function decodeLeafPrefix(
    bytes32 leaf
  ) internal pure returns (uint64 chainid, uint64 nextChainid, uint8 isFallback) {
   ...
   */
  function scratchPackedKeccak256(
    bytes32 a,
    bytes32 b
