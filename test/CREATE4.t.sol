...
contract CREATE4Harness is CREATE4 {
    function packLeafPrefixExternal(uint64 chainid, uint64 nextChainid, uint8 isFallback)
        external
        pure
        returns (bytes32)
    {
    ...
    function decodeLeafPrefixExternal(bytes32 leaf)
        external
        pure
...
contract CREATE4Test is Test {
    ...
    function testDeploysFallbackThroughGapProof() public {
        ...
        address deployed = create4.deployFallback(
            fallbackCase.gapLeafPrefix, fallbackCase.gapLeafHash, gapProof, proof, fallbackInitCode, salt
        );
    ...
    function testDeployFallbackRevertsForInvalidGap() public {
        ...
        create4.deployFallback(alpha.prefix, alpha.initCodeHash, gapProof, proof, fallbackInitCode, salt);
    }
    ...
    function testFuzzPlanMatchesCli(
        ...
        address deployed =
            create4.deploy(copyProof(targetLeaf.proof), targetLeaf.initCode, targetLeaf.nextChainId, computedSalt);
    ...
    function testFuzzFallbackDeploysCorrectly(uint256 seed, uint8 leafCountRaw, uint64 entropy) public {
        ...
        address deployed = create4.deployFallback(
            gapLeaf.prefix, gapLeaf.initCodeHash, gapProof, fallbackProofCopy, fallbackData.initCode, planSaltLocal
        );
    ...
    function testDeployFallbackRevertsForCorruptedProof() public {
        ...
        create4.deployFallback(
            fallbackCase.gapLeafPrefix, fallbackCase.gapLeafHash, gapProof, fallbackProofCopy, fallbackInitCode, salt
        );
    }
    ...
    function testDeployFallbackSingleLeafPlanAllowsFallbackOnOtherChains() public {
        (LeafData memory singleLeaf, FallbackJson memory fallbackData, bytes32 saltLocal, bytes32 planRootLocal) =
            buildSingleLeafPlan();
    ...
    function testDeployFallbackRevertsForCorruptedGapLeafHash() public {
        ...
        vm.expectRevert(CREATE4.MismatchedProof.selector);
        create4.deployFallback(
            fallbackCase.gapLeafPrefix, corruptedHash, gapProof, fallbackProofCopy, fallbackInitCode, salt
        );
    }
    ...
    function testDeployFallbackRevertsForGapPrefixMismatch() public {
        ...
        vm.expectRevert(CREATE4.MismatchedProof.selector);
        create4.deployFallback(alpha.prefix, beta.initCodeHash, gapProof, fallbackProofCopy, fallbackInitCode, salt);
    }
    ...
    function testDeployFallbackRevertsWhenGapUnavailable() public {
        ...
        try create4.deployFallback(
            gapLeaf.prefix, gapLeaf.initCodeHash, gapProof, fallbackProofCopy, fallbackData.initCode, saltLocal
        ) returns (
            address
        ) {
    ...
    function testDeployForwardsValueToPayableLeaf() public {
        ...
        address deployed =
            create4.deploy{value: sendValue}(copyProof(leaf.proof), leaf.initCode, leaf.nextChainId, saltLocal);
    ...
    function testDeployRevertsWhenSendingValueToNonPayableLeaf() public {
        ...
        vm.expectRevert();
        create4.deploy{value: 1 wei}(copyProof(leaf.proof), leaf.initCode, leaf.nextChainId, saltLocal);
    }
    ...
    function writeSpecToCache(string memory contents) internal returns (string memory path) {
        ...
        string memory base = string.concat(
            vm.projectRoot(), "/cache/cli-spec-", bytes32ToHex(specHash), "-", uintToString(specNonce++), ".json"
        );
    ...
    function constantInitCode(uint8 value) internal pure returns (bytes memory) {
        bytes memory runtime = abi.encodePacked(hex"60", bytes1(value), hex"60005260206000f3");
        bytes1 runtimeLength = bytes1(uint8(runtime.length));
        return abi.encodePacked(hex"60", runtimeLength, hex"600c60003960", runtimeLength, hex"6000f3", runtime);
    }
    ...
    function generatePlanSpec(uint256 seed, uint256 leafCount) internal pure returns (string memory) {
        ...
        for (uint256 i = 0; i < leafCount; i++) {
            ...
            chains[i] = ChainInput({chainId: chainId, initCode: constantInitCode(value), label: labelForIndex(i)});
        }
    ...
    function computeCreate3ProxyAddress(address deployer, bytes32 createSalt) internal pure returns (address) {
        bytes32 proxy = keccak256(abi.encodePacked(hex"ff", deployer, createSalt, KECCAK256_PROXY_CHILD_BYTECODE));
    ...
    function computeCreate3Address(address deployer, bytes32 createSalt) internal pure returns (address) {
        ...
        return address(uint160(uint256(keccak256(abi.encodePacked(hex"d694", proxyAddr, hex"01")))));
    }
    ...
    function computeGapTargetExternal(uint64 chainId, uint64 nextChainId, uint64 entropy)
        external
        pure
        returns (uint64)
    {
    ...
    function assertPrefixDecodes(
        bytes32 prefix,
        uint64 expectedChainId,
        uint64 expectedNext,
        uint8 expectedFallbackFlag
    ) internal view {
